...
from model.losses import WeightedHuberLoss, BeamAwareWeightedHuberLoss
...
def _tiny_loader_with_background(num_samples=2, batch_size=2):
    # Half the width is exactly zero (background), half is beam -- so
    # BeamAwareWeightedHuberLoss's mask actually splits into both regions.
    planned = torch.rand(num_samples, 1, 256, 542)
    planned[..., :271] = 0.0
    residual = torch.rand(num_samples, 1, 256, 542) * 0.1
    dataset = TensorDataset(planned, residual)
...
def test_train_runs_one_epoch_with_beam_aware_loss(tmp_path):
    generator = Generator(base_channels=4, num_residual_blocks=1)
    optimizer = build_optimizer(generator)
    criterion = BeamAwareWeightedHuberLoss()

    train_loader = _tiny_loader_with_background()
    val_loader = _tiny_loader_with_background()
